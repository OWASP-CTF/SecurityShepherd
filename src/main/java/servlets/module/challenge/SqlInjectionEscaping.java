...
public class SqlInjectionEscaping extends HttpServlet {
  ...
  /**
   * This SQL Injection Module Class binds user data to a parameterised query so that it can never
   * be interpreted as SQL by the MySQL interpreter
   */
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (Validate.validateSession(ses)) {
      ...
      try {
        String aUserId = Validate.validateParameter(request.getParameter("aUserId"), 64);
        log.debug("User Submitted - " + aUserId);
        String ApplicationRoot = getServletContext().getRealPath("");

        log.debug("Getting Connection to Database");
        String theQuery = "SELECT * FROM customers WHERE customerId = ?";
        try (Connection conn =
                Database.getChallengeConnection(ApplicationRoot, "SqlChallengeEscape");
