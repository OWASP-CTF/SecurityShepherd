    ...
    try {
      ...
      if (Validate.validateSession(ses)) {
        ...
        if (!userId.equals(plusId)) {
          // The nonce guarding this request only had a short set of possible values, so it could
          // be guessed by an off-site page and proved nothing about the user's intent. A request
          // can also name any user, and nothing in it establishes that the named user meant this
          // to happen, so state is no longer changed on behalf of anybody else.
