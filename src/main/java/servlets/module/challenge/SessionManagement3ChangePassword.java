...
public class SessionManagement3ChangePassword extends HttpServlet {
  ...
  /**
   * Function used by Session Management Challenge Three to change the password of the sub
   * application account bound to the user's server side session. The "current" cookie is not used.
   *
   * @param newPassword the password which to use to update the session's account
   */
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (Validate.validateSession(ses)) {
      ...
      try {
        ...
        // The account to update is authoritative server side state, set by SessionManagement3 when
        // a sub application sign in succeeds. The client controlled "current" cookie is ignored,
        // so a user can only ever change the password of the account they are signed in as
        String subName = SessionManagement3.getSubUserFromSession(ses);
        Object passNewObj = request.getParameter("newPassword");
        String subNewPass = new String();
        if (passNewObj != null) {
          subNewPass = (String) passNewObj;
        }
        log.debug("Changing password for session bound sub user: " + subName);
