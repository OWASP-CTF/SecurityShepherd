  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (Validate.validateSession(ses)) {
      ...
      try {
        ...
        // Whose password is being changed is decided by the sub-application login, not by the
        // "current" cookie. That cookie was a double-base64 user name, so any client could
        // point the reset at another account -- including the admin's.
        String subName = (String) ses.getAttribute(SessionManagement3.SUB_APP_USER);
        if (subName == null) {
          log.debug("No sub-application login on this session; refusing password change");
          out.write("<p>" + bundle.getString("reset.failed") + "</p>");
          return;
        }
