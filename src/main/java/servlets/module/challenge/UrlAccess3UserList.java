...
public class UrlAccess3UserList extends HttpServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (Validate.validateSession(ses)) {
      ...
      try {
        // The submitted "currentPerson" cookie used to select which user's row this query
        // looked up, and the query itself unconditionally included every admin row regardless
        // of what was asked for ("... WHERE userRole = 'admin' OR userName = ?"). Together those
        // handed back the super-admin's name to whoever asked, no matter what identity - real,
        // forged, or absent - they presented. This sub-application only ever exposes the guest
        // view; nothing a caller submits can change whose row is returned.
