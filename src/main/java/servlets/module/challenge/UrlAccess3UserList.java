...
public class UrlAccess3UserList extends HttpServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (Validate.validateSession(ses)) {
      ...
      try {
        // The "currentPerson" cookie is client supplied and unauthenticated. The sub schema
        // identity is tracked server side in the session instead.
        String currentUser = (String) ses.getAttribute("urlAccess3CurrentPerson");
        if (currentUser == null) {
          currentUser = "aGuest";
        }
