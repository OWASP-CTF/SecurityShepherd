  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if (Validate.validateSession(ses)) {
        ...
        // A real CSRF-forged request (e.g. an <img> tag on an attacker's page) can never carry
        // the current session's own CSRF cookie/token pair, so requiring both here - the same
        // check CsrfChallengeOne already does for the message-setting action - closes the
        // vulnerability this "Target" endpoint otherwise had none of.
