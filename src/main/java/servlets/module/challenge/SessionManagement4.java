  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if (Validate.validateSession(ses)) {
        ...
        // The "SubSessionID" cookie is client supplied and its sequential value is
        // trivially guessable, so it must never drive an authorisation decision. The sub
        // schema session id is tracked server side in the session instead.
        String decodedCookie = (String) ses.getAttribute("sessionManagement4SubSession");
        if (decodedCookie == null) {
          decodedCookie = "0000000000000001";
          ses.setAttribute("sessionManagement4SubSession", decodedCookie);
        }
