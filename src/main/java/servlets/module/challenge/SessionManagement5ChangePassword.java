...
public class SessionManagement5ChangePassword extends HttpServlet {
  ...
  /**
   * Function used by Session Management Challenge Five to change the password of the account signed
   * in on this session
   *
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (Validate.validateSession(ses)) {
      ...
      try {
        ...
        // The account comes from the sign in this session already proved. The reset used to take
        // the name from the request and authorise it with a base64'd timestamp, which any caller
        // can write, so every account in the sub schema could be reset by anyone.
        Object signedInObj = ses.getAttribute(SessionManagement5.SUB_USER);
        String userName = signedInObj instanceof String ? (String) signedInObj : new String();
        log.debug("userName = " + userName);

        if (userName.isEmpty()) {
          log.debug("No account signed in on this session");
