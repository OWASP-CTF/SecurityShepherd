...
public class SessionManagement5ChangePassword extends HttpServlet {
   ...
   * name. The function requires the unexpired, one-time token issued for that user.
   *
   ...
   * @param resetPasswordToken Server-issued password reset token
   */
        ...
        if (newPass.length() >= 12) {
          ...
          callstmt =
              conn.prepareStatement(
                  "UPDATE users SET userPassword = SHA(?), resetToken = NULL,"
                      + " resetTokenExpires = NULL WHERE userName = ? AND resetToken = SHA2(?,"
                      + " 256) AND resetTokenExpires >= ?");

          callstmt.setString(1, newPass);
          callstmt.setString(2, userName);
          callstmt.setString(3, token);
          callstmt.setLong(4, System.currentTimeMillis());

          log.debug("Executing changePassword");
          int changedRows = callstmt.executeUpdate();
          ...
          htmlOutput =
              "<p>"
                  + bundle.getString(
                      changedRows == 1 ? "changePass.success" : "changePass.funkyToken")
                  + "</p>";
        } else {
