  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if (Validate.validateSession(ses)) {
        ...
        // Privilege check: only a Shepherd admin may access the super-admin result.
        // The cookie-based "currentPerson" path is removed to prevent cookie-forging exploits.
        String sessionRole = (String) ses.getAttribute("userRole");
        boolean isAdmin = "admin".equals(sessionRole);
        String htmlOutput = null;
        if (isAdmin) {
          log.debug("Shepherd admin session detected - granting super-admin club access");
