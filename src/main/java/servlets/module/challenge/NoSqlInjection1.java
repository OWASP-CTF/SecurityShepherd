...
public class NoSqlInjection1 extends HttpServlet {
  ...
  private static final Logger log = LogManager.getLogger(NoSqlInjection1.class);
  ...
  /**
   * Users have to use NoSQL injection to get a specific user (Marlo) gamer ID. The query they are
   * injecting into by default only outputs usernames. The input they enter is also been filtered.
   * theGamerName User name used in database look up.
   */
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (Validate.validateSession(ses)) {
      ...
      ResourceBundle bundle =
          ResourceBundle.getBundle("i18n.servlets.challenges.injection.nosql", locale);
      ...
      try {
        ...
        dbCollectionName =
            MongoDatabase.getMongoChallengeCollName(applicationRoot, "NoSqlInjection1");
        ...
        // Bind the field directly instead of building a $where JS expression from user input
