  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if (Validate.validateSession(ses)) {
        ...
        // The "checksum" cookie is client supplied and unauthenticated, so it must never
        // drive an authorisation decision. The sub schema role is tracked server side in
        // the session instead.
        String decodedCookie = (String) ses.getAttribute("sessionManagement1Role");
        if (decodedCookie == null) {
          decodedCookie = "userRole=user";
          ses.setAttribute("sessionManagement1Role", decodedCookie);
        }
