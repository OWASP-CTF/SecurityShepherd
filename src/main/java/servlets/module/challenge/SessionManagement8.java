    ...
    try {
      ...
      if (Validate.validateSession(ses)) {
        ...
        if (theCookie != null) {
          ...
          // No privilege is derived from this cookie any more. The role is a bare string the client
          // holds, with nothing tying it to anything the server issued, so a user can simply set it
          // to the super-user value. This level has no server-side record of its own roles to check
          // such a claim against, so the claim is no longer honoured at all.
