...
/**
 * Control class responsible for returning a cheat sheet for a module <br>
 * <br>
...
public class GetCheat extends HttpServlet {
  ...
  private static final Logger log = LogManager.getLogger(GetCheat.class);

  /**
   * This method will reject requests if cheat sheet availability is marked as unavailable by
   * administration.
   *
   * @param moduleId
   * @param csrfToken
    ...
    if (Validate.validateSession(ses)) {
      ...
      if (Validate.validateTokens(ses, tokenCookie, tokenParmeter)) {
        if (CheatSheetStatus.showCheat(ses.getAttribute("userRole").toString())) {
          ...
          log.debug(
              ses.getAttribute("userName") + " submitted the following moduleId: " + moduleId);
          if (moduleId != null) {
            result = Getter.getModuleSolution(ApplicationRoot, moduleId, locale);
            if (result != null) {
              out.write(
                  "<div id='theActualCheat' class='cheatBox'>"
                      + "<big style='color:#A878EF;'>"
                      + Encode.forHtml(result[0])
                      + " Cheat</big>"
                      + "<p>"
                      + result[1]
                      + "</div></p><br>");
