...
/**
 * This class is the control structure of the Create class vulnerability <br>
 * <br>
...
public class CreateClass extends HttpServlet {
  ...
  /**
   * Initiated by createClass.jsp. Class is added to the system if valid data has been submitted.
   * Otherwise no class is added to the core database schema. Adding of class to Core Database is
   * handed by Setter.createClass
   *
    ...
    if (Validate.validateAdminSession(ses, tokenCookie, tokenParmeter)) {
      ...
      if (Validate.validateTokens(ses, tokenCookie, tokenParmeter)) {
        ...
        try {
          ...
          notEmpty = notNull && !classYear.isEmpty() && !className.isEmpty();
          log.debug("Validating Year");
          validYear = notNull && Validate.isValidClassYear(classYear);
          log.debug("Validating Name");
          classValidate =
              notNull
                  && className.length() > 4
                  && className.length() <= 32
                  && className.codePoints().noneMatch(Character::isISOControl);
          ...
          if (notNull && notEmpty && validYear && classValidate) {
            ...
            if (Setter.classCreate(ApplicationRoot, className, classYear)) {
              reponseMessage =
                  "<h3 class=\"title\">Class Created</h3>"
                      + "<p>"
                      + "Class <a>"
                      + Encode.forHtml(className)
                      + "</a> of <a>"
                      + Encode.forHtml(classYear)
                      + "</a> created successfully."
                      + "</p>";
            } else {
              reponseMessage =
                  "<h3 class=\"title\">Class Create Failure</h3><p>Sorry but an error Occurred!"
                      + " Please try again. If the problem persists please contact an"
                      + " administrator!<p>";
            }
      ...
      out.print(
          "<h3 class=\"title\">Class Create Failure</h3><p><font color=\"red\">An error Occurred!"
