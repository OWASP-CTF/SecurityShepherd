...
/**
 * Control class of the "UnSuspend User" functionality <br>
 * <br>
...
public class UnSuspendUser extends HttpServlet {
  ...
  private static final Logger log = LogManager.getLogger(UnSuspendUser.class);
        ...
        try {
          ...
          if (notNull && validPlayer) {
            ...
            if (Setter.unSuspendUser(ApplicationRoot, player)) {
              ...
              // Adding to Kick List So they application signs them out if they are currently logged
              // in
              UserKicker.removeFromKicklist(userName);
              reponseMessage +=
                  "<a>" + Encode.forHtml(userName) + "</a> has been enabled for play<br>";
            } else {
              reponseMessage +=
                  "<font color='red'>User could not be suspended. Please try again.</font><br/>";
            }
            out.print(
                "<h3 class=\"title\">Suspension Lifted</h3>" + "<p>" + reponseMessage + "<p>");
          } else {
